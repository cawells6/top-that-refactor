...
describe('rank', () => {
    ...
    test('rank returns 0 for unrankable or invalid card values', () => {
        ...
        expect(rank(createCard(null))).toBe(0);
        expect(rank(createCard(undefined))).toBe(0);
        expect(rank(createCard('xyz'))).toBe(0); // normalizeCardValue('xyz') -> 'xyz', rank('xyz') -> 0
        expect(rank({ value: {}, suit: 'hearts' })).toBe(0); // normalizeCardValue({}) -> '[object object]', rank -> 0
    });
});
...
describe('isFourOfAKind', () => {
    const createHand = (values, suitPrefix = 's') => {
        ...
        return values.map((value, index) => ({
            value: value, // Cast needed if null/undefined are possible in input array for test
    ...
    test('returns false for not four of a kind', () => {
        ...
        const hand = createHand(['ACE', 'King', 'Queen', 'Jack']);
        expect(isFourOfAKind(hand)).toBe(false);
    });
    ...
    test('returns false if any card value results in null/undefined after normalization', () => {
        ...
        const handWithInvalidValueForNorm = [
            { value: null, suit: 's1' }, // normalizeCardValue(null) -> null
            { value: 'K', suit: 's2' },
            { value: 'K', suit: 's3' },
            { value: 'K', suit: 's4' },
        ];
        expect(isFourOfAKind(handWithInvalidValueForNorm)).toBe(false);
        const handWithOtherInvalid = [
            { value: 'K', suit: 's1' },
            { value: undefined, suit: 's2' }, // normalizeCardValue(undefined) -> undefined
            { value: 'K', suit: 's3' },
            { value: 'K', suit: 's4' },
        ];
    ...
    test('isFourOfAKind works for non-keyword numbers that normalize to same string', () => {
        const handThrees = createHand([3, '3', 3, '3']); // All normalize to "3"
        expect(isFourOfAKind(handThrees)).toBe(true);
    });
    test('isFourOfAKind for specific keywords like "two"', () => {
        const handTwos = createHand(['2', 2, 'two', '2']); // All normalize to "two"
