...
(function() {
  ...
  function cleanupElementStyles(element) {
    ...
    if (element.classList.contains('rule-card-img') || 
    ...
    if (element.classList.contains('card-image-container') || 
        element.classList.contains('card-symbol')) {
  ...
  function cleanupAllElements() {
    // Process card images
    document.querySelectorAll('.rule-card-img, .card-symbol, [data-fixed-loader]')
      .forEach(cleanupElementStyles);
      
    // Process containers
    document.querySelectorAll('.card-image-container, .card-symbol')
      .forEach(cleanupElementStyles);
  }
      ...
      if (mutation.addedNodes && mutation.addedNodes.length > 0) {
        mutation.addedNodes.forEach((node) => {
          ...
          if (node.nodeType === 1) {
            ...
            node.querySelectorAll('.rule-card-img, .card-symbol, [data-fixed-loader], .card-image-container')
              .forEach(cleanupElementStyles);
          }
        });
      }
