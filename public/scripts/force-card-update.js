...
function forceCardUpdate() {
  ...
  if (window.fixedCardLoaderDiagnostics) {
    if (typeof window.fixedCardLoaderDiagnostics.resetCards === 'function') {
      ...
      return;
    } else if (typeof window.fixedCardLoaderDiagnostics.clearCache === 'function') {
  ...
  if (!hasDataAttributes) {
    console.warn('[FORCE-UPDATE] ⚠️ No data-card-value attributes found - cards might not render correctly');
  }
  
  // Remove any existing card images
  const existingCards = document.querySelectorAll('.rule-card-img');
  console.log(`[FORCE-UPDATE] Removing ${existingCards.length} existing card images`);
  ...
  if (typeof window.updateRuleCardsWithImages === 'function') {
    ...
    try {
      window.updateRuleCardsWithImages()
        .then(result => {
          console.log(`[FORCE-UPDATE] Card update ${result ? 'succeeded' : 'failed'}`);
          
          // Check if the update succeeded but didn't produce any visible cards
          setTimeout(() => {
            const renderedCards = document.querySelectorAll('.rule-card-img[data-fixed-loader="true"]');
            if (renderedCards.length === 0) {
              console.warn('[FORCE-UPDATE] ⚠️ No cards were rendered - trying again with delay');
      ...
      console.error('[FORCE-UPDATE] Error calling updateRuleCardsWithImages:', err);
    }
  } else {
    console.error('[FORCE-UPDATE] updateRuleCardsWithImages function not found');
  }
}
    ...
    const observer = new MutationObserver((mutations) => {
      for (const mutation of mutations) {
        if (mutation.attributeName === 'class') {
          ...
          if (target instanceof HTMLElement && !target.classList.contains('modal--hidden')) {
            console.log('[FORCE-UPDATE] Rules modal shown, forcing card update');
