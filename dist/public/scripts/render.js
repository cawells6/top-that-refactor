 ...
 */
export function createCenterPiles(gameState) {
    ...
    if (gameState.discard && gameState.discard.length) {
...
export function renderGameState(gameState) {
    console.log('Rendering game state:', gameState); // Added for debugging
    ...
    function seatFor(idx) {
        if (playerCount === 2)
            return idx === meIdx ? 'bottom' : 'top';
        if (playerCount === 3) {
            if (idx === meIdx)
                return 'bottom';
            if ((idx - meIdx + playerCount) % playerCount === 1)
                return 'left';
            return 'right';
        }
        if (playerCount === 4) {
            ...
            if ((idx - meIdx + playerCount) % playerCount === 1)
                return 'left';
            if ((idx - meIdx + playerCount) % playerCount === 2)
                return 'top';
            return 'right';
        }
        return 'bottom';
    ...
    players.forEach((p, idx) => {
        ...
        if (seat === 'bottom' && p.id === myId)
            panel.id = 'my-area';
        if (p.isComputer)
            panel.classList.add('computer-player');
        if (p.disconnected)
            panel.classList.add('disconnected');
        if (p.id === players[gameState.currentPlayerIndex]?.id) {
        ...
        if (seat === 'left')
            panel.classList.add('rotate-right');
        if (seat === 'right')
            panel.classList.add('rotate-left');
        // Name header (banner)
        ...
        if (p.hand && p.hand.length > 0) {
            ...
            for (let i = 0; i < p.hand.length; i++) {
                const card = p.hand[i];
                const canInteract = players[gameState.currentPlayerIndex]?.id === myId;
            ...
            const displayCount = Math.min(p.hand.length, 3);
        ...
        handLabel.textContent = `Hand${p.id === myId ? '' : ' (' + (p.hand?.length || 0) + ')'}`;
        ...
        if (p.upCards && p.upCards.length > 0) {
            p.upCards.forEach((c, i) => {
                ...
                col.className = 'stack';
                const downCard = cardImg({ back: true }, p.id === myId &&
                    players[gameState.currentPlayerIndex]?.id === myId &&
                ...
                if (downCardImg && downCardImg instanceof HTMLImageElement) {
                    ...
                    if (p.id === myId)
                        downCardImg.dataset.idx = String(i + 2000);
                }
                const upCard = cardImg(c, p.id === myId && players[gameState.currentPlayerIndex]?.id === myId && p.hand.length === 0);
                ...
                if (upCardImg && upCardImg instanceof HTMLImageElement) {
                    ...
                    if (p.id === myId)
                        upCardImg.dataset.idx = String(i + 1000);
                }
        ...
        else if (p.downCards && p.downCards.length > 0) {
            for (let i = 0; i < p.downCards.length; i++) {
                const col = document.createElement('div');
                col.className = 'stack';
                const downCard = cardImg({ back: true }, p.id === myId &&
                    players[gameState.currentPlayerIndex]?.id === myId &&
        ...
        if (seat === 'bottom' && slotBottom)
            slotBottom.appendChild(panel);
        else if (seat === 'top' && slotTop)
            slotTop.appendChild(panel);
        else if (seat === 'left' && slotLeft)
            slotLeft.appendChild(panel);
        else if (seat === 'right' && slotRight)
            slotRight.appendChild(panel);
    });
    // Draw center piles
    ...
    function tryRunEffect() {
        ...
        function runEffect() {
            ...
            if (type === 'two' || cardValue == 2)
                ...
                src = '/src/shared/invalid play-icon.png';
            else if (type === 'take')
                src = '/src/shared/take pile-icon.png';
