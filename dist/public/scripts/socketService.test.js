...
describe('socketService', () => {
    ...
    it('calls showLobbyForm if not rejoining on connect', () => {
        ...
        const connectHandler = state.socket.on.mock.calls.find(([event]) => event === 'connect')[1];
        connectHandler();
        expect(uiManager.showLobbyForm).toHaveBeenCalled();
    });
    it('calls showWaitingState on LOBBY event', () => {
        initializeSocketHandlers();
        const lobbyHandler = state.socket.on.mock.calls.find(([event]) => event === LOBBY)[1];
    ...
    it('calls renderGameState and showGameTable on STATE_UPDATE', () => {
        initializeSocketHandlers();
        const stateUpdateHandler = state.socket.on.mock.calls.find(([event]) => event === STATE_UPDATE)[1];
    ...
    it('calls showError on err event', () => {
        initializeSocketHandlers();
        const errHandler = state.socket.on.mock.calls.find(([event]) => event === 'err')[1];
        errHandler('fail!');
